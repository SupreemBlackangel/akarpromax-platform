...
export default function OfficeAppPromotion({ labels, cta, description, href, onCta }: OfficeAppPromotionProps) {
  ...
  const action = href ? (
    <a
      href={href}
      className="inline-flex h-12 select-none items-center justify-center gap-[var(--space-2)] whitespace-nowrap rounded-[var(--radius-md)] bg-[color:var(--color-primary)] px-[var(--space-8)] text-body font-medium leading-none text-[color:var(--color-primary-foreground)] transition-[background-color,box-shadow] duration-[var(--motion-fast)] ease-[var(--easing-standard)] hover:bg-[color:var(--color-primary-hover)] focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus)]"
    >
      {cta}
    </a>
  ) : (
  ...
  return (
    <section aria-label={labels.officeAppAria} className="border-y border-[color:var(--color-border)] bg-[color:var(--color-surface)]">
      <PageContainer className="flex flex-col gap-[var(--space-5)] py-[var(--space-6)] sm:flex-row sm:items-center sm:justify-between">
        <p className="max-w-prose text-body text-[color:var(--color-text-secondary)]">{description}</p>
        {hasAction && action}
      </PageContainer>
    </section>
  );
