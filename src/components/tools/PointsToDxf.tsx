...
export function PointsToDxf({ locale }: Props) {
  ...
  return (
    <ToolCalculatorShell
      title={locale === "ar" ? "تحويل النقاط إلى DXF" : "Points → DXF"}
      subtitle={locale === "ar"
      ...
      <div className="tc-tool-inner">
        ...
        <div className="tc-settings-row">
          ...
          <label className="flex items-center gap-2 text-xs min-h-[44px]">
            <input type="checkbox" checked={includeLabels} onChange={(e) => setIncludeLabels(e.target.checked)} className="accent-blue-600 w-4 h-4" />
            {locale === "ar" ? "أرقام النقاط" : "Point labels"}
          </label>
          <label className="flex items-center gap-2 text-xs min-h-[44px]">
            <input type="checkbox" checked={closePolygon} onChange={(e) => setClosePolygon(e.target.checked)} className="accent-blue-600 w-4 h-4" />
        ...
        {cleanDoc && (
          <>
            ...
            <button type="button" className="tc-copy-btn min-h-[44px]" onClick={copyDxf}>
