...
export function BeamCalc({ locale }: Props) {
  ...
  const loadExample = () => setInput({ b: 0.3, h: 0.6, length: 6 });
  const set = (field: keyof BeamInput) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div dir={dir}>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {[{ field: "b" as const, label: t("b") }, { field: "h" as const, label: t("h") }, { field: "length" as const, label: t("length") }].map(({ field, label }) => (
          <div key={field}>
