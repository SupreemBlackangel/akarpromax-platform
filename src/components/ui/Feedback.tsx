...
export function EmptyState({
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center gap-[var(--space-3)] rounded-[var(--radius-lg)] border border-dashed border-[color:var(--color-border-strong)] bg-[color:var(--color-surface-muted)]/50 p-[var(--space-12)] text-center", className)}>
      ...
      <h3 className="text-h3 font-semibold text-[color:var(--color-text-primary)]">{title}</h3>
      {description && <p className="max-w-sm text-body-sm text-[color:var(--color-text-muted)]">{description}</p>}
    ...
    <div className={cn("w-full", className)}>
      ...
      {retry && onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="mt-[var(--space-3)] inline-flex h-8 items-center rounded-[var(--radius-md)] px-[var(--space-4)] text-body-sm font-medium text-[color:var(--color-primary)] transition-colors duration-[var(--motion-fast)] hover:bg-[color:var(--color-primary-soft)] focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus)]"
        >
          {retry}
        </button>
      )}
...
export function LoadingState({
  ...
  return (
    <div role="status" className={cn("flex items-center justify-center gap-[var(--space-3)] p-[var(--space-8)]", className)}>
      ...
      {label && <span className="text-body-sm text-[color:var(--color-text-muted)]">{label}</span>}
