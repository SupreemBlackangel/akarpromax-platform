    ...
    <form id="property-form" onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card id="step-media" data-section="media" data-step="5" className="rounded-2xl border-[var(--color-border)] shadow-sm hover:shadow-md transition">
        ...
        <CardContent className="space-y-3">
          ...
          {formData.media.map((media, index) => (
            <div key={index} className="flex gap-2 items-center">
              {media.type === 'image' && media.url ? (
                ...
                <span className="h-10 w-14 shrink-0 rounded-lg bg-[var(--color-surface-muted)] border border-[var(--color-border)] flex items-center justify-center text-label font-bold text-[var(--color-text-muted)]">{media.type === 'video' ? 'فيديو' : '—'}</span>
              )}
              {index === 0 && <span className="shrink-0 rounded-full bg-[var(--color-primary-soft)] border border-[var(--color-border)] px-2 py-0.5 text-label font-bold text-[var(--color-primary)]">غلاف</span>}
              <input
                type="text"
                value={media.url}
          ...
          <p className="text-label text-[var(--color-text-muted)] font-medium">تُحوَّل الصور تلقائيًا إلى WebP بمقاس موحد لسرعة التصفح.</p>
        </CardContent>
      </Card>
