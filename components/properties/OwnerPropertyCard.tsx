  ...
  return (
    <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm overflow-hidden flex flex-col">
      <div className="relative h-40 bg-gray-100 dark:bg-gray-800">
        ...
        <span className={`absolute top-3 right-3 rounded-full px-3 py-1 text-label font-bold ${meta.cls}`}>{meta.label}</span>
        {property.dealType && (
          <span className="absolute top-3 left-3 rounded-full bg-black/60 px-3 py-1 text-label font-bold text-white">
      ...
      <div className="p-4 flex-1 flex flex-col gap-2" dir="rtl">
        <h3 className="text-sm font-bold text-[var(--color-text-primary)] line-clamp-1">{property.titleAr || property.titleEn || 'عقار'}</h3>
        ...
        <div className="flex items-center gap-3 text-label text-[var(--color-text-muted)] font-medium">
        ...
        {status === 'rejected' && property.rejectedReason && (
          <p className="flex items-start gap-1.5 rounded-lg bg-red-50 dark:bg-red-900/20 px-3 py-2 text-label font-semibold text-red-700 dark:text-red-300">
            <AlertCircle className="h-3.5 w-3.5 shrink-0 mt-0.5" />
            سبب الرفض: {property.rejectedReason}
          </p>
        )}
        {message && <p className="text-label font-semibold text-[var(--color-text-muted)]" role="status">{message}</p>}

        <div className="mt-auto flex flex-wrap gap-2 pt-2">
          {status === 'approved' && (
            <Link href={`/properties/${property.id}`} className="rounded-lg border border-[var(--color-border)] px-3 py-1.5 text-label font-bold text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-muted)] transition">
              عرض الصفحة العامة
            </Link>
          )}
          {canEdit && (
            <Link href={`/dashboard/properties/${property.id}/edit`} className="inline-flex items-center gap-1 rounded-lg bg-[var(--color-primary-soft)] px-3 py-1.5 text-label font-bold text-[var(--color-primary)] hover:opacity-90 transition">
          ...
          {canSubmit && (
            <button
              ...
              disabled={busy !== null}
              className="inline-flex items-center gap-1 rounded-lg bg-green-600 px-3 py-1.5 text-label font-bold text-white hover:bg-green-700 disabled:opacity-50 transition"
            >
          ...
          <button
            ...
            onClick={remove}
            disabled={busy !== null}
            className="inline-flex items-center gap-1 rounded-lg bg-red-50 dark:bg-red-900/20 px-3 py-1.5 text-label font-bold text-red-600 hover:bg-red-100 disabled:opacity-50 transition"
          >
