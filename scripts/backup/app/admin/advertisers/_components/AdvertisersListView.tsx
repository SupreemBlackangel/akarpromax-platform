...
export default function AdvertisersListView() {
  ...
  function fetchAll() {
    setLoading(true);
    fetch("/api/advertiser-profiles").then(r => r.ok ? r.json() : []).then(d => setProfiles(d)).finally(() => setLoading(false));
  }
  ...
  const filtered = useMemo(() => {
    return profiles.filter(p => {
      ...
      if (search) {
        ...
        if (!p.companyNameAr.toLowerCase().includes(q) && !p.companyNameEn.toLowerCase().includes(q) && !(p.email || "").toLowerCase().includes(q) && !(p.phone || "").includes(q) && !p.advertiserCode.toLowerCase().includes(q)) return false;
      }
      return true;
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">المعلنون</h1>
        <div className="flex gap-2">
          <Link href="/admin/advertisers/new" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm transition-colors">
            + إضافة معلن
          </Link>
          <Link href="/admin/advertisers/management" className="px-4 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg text-sm transition-colors">
            إدارة الحملات
          </Link>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-xl p-8 text-center border border-gray-200 dark:border-gray-700">
          <p className="text-gray-500 dark:text-gray-400">لا يوجد معلنون</p>
        </div>
      ) : (
                ...
                <tr key={profile.id} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                  ...
                  <td className="p-3">
                    <div className="flex gap-1">
                      <Link href={`/admin/advertisers/${profile.id}`} className="px-2 py-1 text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors">عرض</Link>
                      <Link href={`/admin/advertisers/${profile.id}/edit`} className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">تعديل</Link>
                    </div>
                  </td>
                </tr>
              ))}
