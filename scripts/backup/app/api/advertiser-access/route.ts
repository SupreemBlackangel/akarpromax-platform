...
import { getSessionIdentity, hasPermission, type SponsorRole } from "@/lib/identity-auth";
...
export async function GET() {
  ...
  if (!hasPermission(identity, PERMISSIONS.USERS_VIEW)) {
...
export async function POST(request: NextRequest) {
  const identity = await getSessionIdentity();
  if (!hasPermission(identity, PERMISSIONS.USERS_CREATE) && !hasPermission(identity, PERMISSIONS.USERS_UPDATE)) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
  ...
  await db.prepare(
    `INSERT INTO audit_logs (id, actor_user_id, action, entity_type, entity_id, metadata)
     VALUES (?1, ?2, 'advertiser.access.updated', 'sponsor_access', ?3, ?4)`,
  )
...
export async function DELETE(request: NextRequest) {
  const identity = await getSessionIdentity();
  if (!hasPermission(identity, PERMISSIONS.USERS_DELETE)) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
  ...
  await db.prepare(
    `INSERT INTO audit_logs (id, actor_user_id, action, entity_type, entity_id, metadata)
     VALUES (?1, ?2, 'advertiser.access.deleted', 'sponsor_access', ?3, ?4)`,
  )
