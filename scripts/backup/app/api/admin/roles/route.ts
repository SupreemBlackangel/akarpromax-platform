...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    assignableRoles: assignableRoles.map((r) => ({
      id: r,
      nameAr: ROLE_CATALOG[r].nameAr,
      nameEn: ROLE_CATALOG[r].nameEn,
      descriptionAr: ROLE_CATALOG[r].descriptionAr,
      permissionCount: ROLE_CATALOG[r].permissions.length,
    })),
  });
}
...
export async function PATCH(request: NextRequest) {
  ...
  if (!assignableRoles.includes(newRole as SponsorRole)) {
    return NextResponse.json({ error: "Invalid role" }, { status: 400 });
  }

  const db = await getRuntimeDb();
  const existing = await db.prepare(
    "SELECT id, role FROM sponsor_access WHERE id = ?1 LIMIT 1",
  ...
  await db.prepare(
    `UPDATE sponsor_access SET role = ?1, updated_at = CURRENT_TIMESTAMP WHERE id = ?2`,
  ...
  await db.prepare(
    ...
    identity.email,
    userId,
    JSON.stringify({ from: existing.role, to: newRole }),
  ).run();
