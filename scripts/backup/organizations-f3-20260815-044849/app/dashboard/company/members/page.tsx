...
export default async function CompanyMembersPage() {
  ...
  if (!session) {
    return (
      <CompanyWorkspaceShell activeTab="members">
  ...
  return (
    <CompanyWorkspaceShell activeTab="members">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-black text-gray-900 dark:text-white">أعضاء الشركة</h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{company.nameAr ?? company.nameEn}</p>
        </div>
        {isAdmin && (
          <Link href="/dashboard/company/members/invite">
                ...
                <tr key={orgMember.id} className="border-b border-gray-50 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800/40">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-purple-100 font-bold text-purple-600 dark:bg-purple-900/40 dark:text-purple-300">
                        {user.name?.[0] ?? "م"}
                      </div>
                      <div>
                  ...
                  <td className="px-4 py-3">
                    <span className="rounded bg-purple-50 px-2 py-1 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">
