...
export default function ServicesCatalogPage() {
  ...
  const inputClass = "h-12 w-full bg-transparent text-sm text-[var(--color-text-primary)] outline-none placeholder:text-[var(--color-text-muted)] dark:text-[var(--color-surface)]";
      ...
      <PublicPageShell locale={locale} copy={copy} viewer={viewer} country={country} city={city} currentPath="/services/catalog" adLayout={{ mode: "standard", family: "services" }} onLogin={() => openLogin("login")} onLogout={handleLogout}>
        <main dir={dir} className="pb-12 pt-6">
          ...
          <section className="mt-4 rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm dark:border-[var(--color-border)] dark:bg-[var(--color-surface)] md:p-7">
            ...
            <h1 className="mt-1 text-3xl font-black text-[var(--color-text-primary)] dark:text-[var(--color-surface)]">{isArabic ? "ابحث في جميع المهن والمحترفين" : "Search every service and professional"}</h1>
          ...
          <section className="mt-12">
            <div className="mb-4 flex items-end justify-between gap-4"><div><h2 className="flex items-center gap-2 text-xl font-black text-[var(--color-text-primary)] dark:text-[var(--color-surface)]"><Users className="h-5 w-5 text-[var(--color-primary)]" />{isArabic ? "محترفون مناسبون" : "Matching professionals"}</h2><p className="mt-1 text-xs text-[var(--color-text-muted)]">{visibleProviders.length} {isArabic ? "ملف معتمد" : "approved profiles"}</p></div><Link href="/providers/apply" className="text-xs font-black text-[var(--color-success)] hover:underline dark:text-[var(--color-success)]">{isArabic ? "سجّل مهنتك" : "List your profession"}</Link></div>
