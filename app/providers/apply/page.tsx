    ...
    <>
      <PublicPageShell locale={locale} copy={copy} viewer={viewer} country={country} city={city} currentPath="/providers/apply" adLayout={{ mode: "standard", family: "providers" }} onLogin={() => openLogin("login")} onLogout={handleLogout}>
        <main dir={dir} className="space-y-8 pb-12 pt-6">
          <Link href="/services" className="inline-flex items-center gap-1.5 text-sm font-bold text-[var(--color-primary)] hover:underline dark:text-[var(--color-primary)]">{isArabic ? "العودة إلى سوق الخدمات" : "Back to services"}<ArrowLeft className="h-4 w-4" /></Link>
          ...
          <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-blue-950 to-blue-800 text-white">
            <div className="grid gap-8 px-6 py-9 md:grid-cols-[1fr_0.8fr] md:px-9 md:py-12">
              <div><div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-[var(--color-surface)]/10 px-3 py-1.5 text-xs font-bold"><BriefcaseBusiness className="h-4 w-4 text-[var(--color-success)]" />{isArabic ? "نمِّ أعمالك مع عقار بروماكس" : "Grow with AkarProMax"}</div><h1 className="mt-4 text-3xl font-bold leading-tight md:text-4xl">{isArabic ? "حوّل مهنتك إلى ملف موثوق وفرص عمل حقيقية" : "Turn your profession into a trusted profile and real opportunities"}</h1><p className="mt-4 max-w-xl text-sm leading-7 text-[var(--color-primary)]/80/80">{isArabic ? "اختر المهن التي تقدمها، حدّد منطقتك ونطاق خدمتك وأسعارك، وأرفق تراخيصك وأعمالك السابقة. بعد المراجعة تصلك الطلبات المطابقة مباشرة." : "Choose your services, service area and pricing, then submit licenses and previous work for review."}</p></div>
              <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-1">{[[BadgeCheck, isArabic ? "اعتماد مهني" : "Professional verification", isArabic ? "شارة اعتماد بعد مراجعة الملف" : "A badge after profile review"], [MapPin, isArabic ? "طلبات حسب منطقتك" : "Local matching", isArabic ? "مطابقة بالمهنة والموقع والنطاق" : "Matching by service and area"], [Star, isArabic ? "سمعة قابلة للنمو" : "Build your reputation", isArabic ? "تقييمات حقيقية بعد كل مهمة" : "Verified reviews after each job"]].map(([Icon, title, text]) => { const ItemIcon = Icon as typeof BadgeCheck; return <div key={String(title)} className="flex gap-3 rounded-2xl border border-white/10 bg-[var(--color-surface)]/5 p-3.5"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-400/15 text-[var(--color-success)]"><ItemIcon className="h-5 w-5" /></span><div><p className="text-sm font-bold">{String(title)}</p><p className="mt-0.5 text-xs text-[var(--color-primary)]/80/60">{String(text)}</p></div></div>; })}</div>
            </div>
          </section>

          {!registrationAllowed ? <section className="rounded-2xl border border-[var(--color-warning)]/40 bg-[var(--color-warning-soft)] p-6 text-center dark:bg-[var(--color-warning-soft)]/30"><ShieldCheck className="mx-auto h-9 w-9 text-[var(--color-warning)]" /><h2 className="mt-3 text-lg font-bold text-[var(--color-text-primary)] dark:text-[var(--color-text-primary)]">{isArabic ? "التسجيل متوقف مؤقتًا" : "Registration is temporarily paused"}</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)] dark:text-[var(--color-text-muted)]">{isArabic ? "يمكن للمدير إعادة فتح التسجيل من لوحة إدارة سوق الخدمات." : "The marketplace administrator can reopen applications."}</p></section> : <>
