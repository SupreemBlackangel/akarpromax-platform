    ...
    <>
      <PublicPageShell locale={locale} copy={copy} viewer={viewer} country={country} city={city} currentPath="/providers/apply" adLayout={{ mode: "standard", family: "providers" }} onLogin={() => openLogin("login")} onLogout={handleLogout}>
        <main dir={dir} className="space-y-8 pb-12 pt-6">
          ...
          {!registrationAllowed ? <section className="rounded-2xl border border-amber-200 bg-[var(--accent-soft)] p-6 text-center dark:border-amber-900 dark:bg-[var(--accent-soft)]/30"><ShieldCheck className="mx-auto h-9 w-9 text-[var(--accent)]" /><h2 className="mt-3 text-lg font-black text-[var(--color-text-primary)] dark:text-[var(--color-text-primary)]">{isArabic ? "التسجيل متوقف مؤقتًا" : "Registration is temporarily paused"}</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)] dark:text-[var(--color-text-muted)]">{isArabic ? "يمكن للمدير إعادة فتح التسجيل من لوحة إدارة سوق الخدمات." : "The marketplace administrator can reopen applications."}</p></section> : <>
