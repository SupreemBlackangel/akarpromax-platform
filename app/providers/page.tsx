    ...
    <>
      <PublicPageShell locale={locale} copy={copy} viewer={viewer} country={country} city={city} currentPath="/providers" adLayout={{ mode: "standard", family: "providers" }} onLogin={() => openLogin("login")} onLogout={handleLogout}>
        <main dir={dir} className="space-y-8 pb-12 pt-6">
          ...
          <section>
            <div className="mb-5 flex flex-wrap items-end justify-between gap-4"><div><h2 className="flex items-center gap-2 text-xl font-bold text-[var(--color-text-primary)] dark:text-[var(--color-text-primary)]"><Users className="h-5 w-5 text-[var(--color-primary)]" />{isArabic ? "المحترفون" : "Professionals"}</h2><p className="mt-1 text-sm text-[var(--color-text-muted)]">{loading ? (isArabic ? "جارٍ البحث..." : "Searching...") : `${filtered.length} ${isArabic ? "نتيجة مطابقة" : "matching results"}`}</p></div><Link href="/providers/apply" className="rounded-xl bg-[var(--color-success)] px-4 py-2.5 text-xs font-bold text-white hover:bg-[var(--color-success)]/80">{isArabic ? "سجّل كحرفي أو شركة" : "Join as a professional"}</Link></div>
            {loading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 6 }, (_, index) => <div key={index} className="h-48 animate-pulse rounded-2xl bg-[var(--color-background)] dark:bg-[var(--color-surface)]" />)}</div> : filtered.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{filtered.map((provider, index) => <ProviderCard key={provider.id} provider={provider} locale={locale} index={index} />)}</div> : <div className="rounded-2xl border border-dashed border-[var(--color-border)] bg-[var(--color-surface-muted)] py-12 text-center dark:border-[var(--color-border)] dark:bg-[var(--color-surface)]/50"><ShieldCheck className="mx-auto h-9 w-9 text-[var(--color-text-muted)]" /><h3 className="mt-3 font-bold text-[var(--color-text-secondary)] dark:text-[var(--color-surface-muted)]">{isArabic ? "لا توجد ملفات مطابقة الآن" : "No matching profiles yet"}</h3><p className="mt-1 text-sm text-[var(--color-text-muted)]">{isArabic ? "جرّب توسيع البحث أو انشر طلب خدمة ليصل إلى المحترفين المناسبين." : "Broaden your search or post a service request."}</p><Link href="/service-requests/new" className="mt-4 inline-block text-sm font-bold text-[var(--color-primary)] hover:underline dark:text-[var(--color-primary)]">{isArabic ? "انشر طلب خدمة" : "Post a request"}</Link></div>}
          </section>
        </main>
      </PublicPageShell>
      {AccountDialog}
