...
export default function NewSponsorForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || "فشل إنشاء الراعي");
      }
      const profile = await res.json();
      router.push(`/admin/sponsors/${profile.id}`);
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6" dir="rtl">
      <h1 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">إضافة راعٍ جديد</h1>
