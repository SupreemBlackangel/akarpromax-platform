...
export default function AdvertiserRequestsView() {
  ...
  function fetchRequests() {
    setLoading(true);
    fetch("/api/advertiser-profiles?status=pending")
  ...
  async function updateStatus(id: string, status: string) {
    setActionLoading(id);
    try {
      await fetch("/api/advertiser-profiles", {
        method: "PATCH",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6" dir="rtl">
      <h1 className="text-2xl font-bold mb-6 text-gray-900 dark:text-[var(--color-text-primary)]">طلبات اعتماد المعلنين</h1>
            ...
            <div key={profile.id} className="bg-[var(--color-surface)] dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <Link href={`/admin/advertisers/${profile.id}`} className="font-semibold text-gray-900 dark:text-[var(--color-text-primary)] hover:text-[var(--color-primary)] dark:hover:text-blue-400">
