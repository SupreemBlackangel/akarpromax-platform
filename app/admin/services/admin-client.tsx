...
type MarketSettings = Record<string, string | number | boolean> & {
  ...
  matchWaveSize: number; maxRequestRenewals: number; requestBlockDays: number; offerValidityHours: number;
};
type CategoryForm = {
...
export default function ServicesAdminClient() {
  ...
  const stats = [
    ["بانتظار المراجعة", overview?.pendingProviders ?? 0, "text-[var(--color-warning)]"], ["محترفون معتمدون", overview?.approvedProviders ?? 0, "text-[var(--color-success)]"],
    ...
    <>
      <main dir={dir} className="p-4 md:p-6">
        ...
        {!loading && tab === "overview" && <div className="space-y-6">
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{stats.map(([label, value, color]) => <div key={String(label)} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 dark:border-[var(--color-border)] dark:bg-[var(--color-surface)]"><p className="text-sm font-bold text-[var(--color-text-muted)]">{label}</p><p className={`mt-2 text-3xl font-bold ${color}`}>{value}</p></div>)}</section>
          <section className="grid gap-4 lg:grid-cols-2"><AdminPanel title="أحدث طلبات الخدمات" action={<button onClick={() => setTab("operations")} className="text-xs font-bold text-[var(--color-primary)]">عرض الكل</button>}><SimpleRows rows={snapshot.recentRequests.slice(0, 6)} empty="لا توجد طلبات" render={(row) => <><div><p className="text-sm font-bold text-[var(--color-text-primary)] dark:text-[var(--color-surface-muted)]">{String(row.title || row.reference_number || "طلب خدمة")}</p><p className="text-xs text-[var(--color-text-muted)]">{String(row.category_name_ar || "غير مصنف")} · {formatDate(String(row.created_at || ""))}</p></div><Status value={String(row.status || "")} /></>} /></AdminPanel><AdminPanel title="أحدث ملفات المحترفين" action={<button onClick={() => setTab("providers")} className="text-xs font-bold text-[var(--color-primary)]">إدارة الملفات</button>}><SimpleRows rows={snapshot.recentProviders.slice(0, 6)} empty="لا توجد ملفات" render={(row) => <><div><p className="text-sm font-bold text-[var(--color-text-primary)] dark:text-[var(--color-surface-muted)]">{row.business_name || row.display_name_ar || "مقدم خدمة"}</p><p className="text-xs text-[var(--color-text-muted)]">{row.governorate || row.city_id || "—"}</p></div><Status value={row.status} /></>} /></AdminPanel></section>
        ...
        {!loading && tab === "content" && settings && <section className="space-y-5"><AdminPanel title="محتوى واجهة السوق" description="كل تعديل هنا ينعكس على الصفحة العامة دون الحاجة لتعديل برمجي."><div className="grid gap-4 md:grid-cols-2"><Field label="الشارة العلوية (عربي)" value={String(settings.heroKickerAr)} onChange={(value) => setSettings({ ...settings, heroKickerAr: value })} /><Field label="Top label (English)" value={String(settings.heroKickerEn)} onChange={(value) => setSettings({ ...settings, heroKickerEn: value })} /><Field label="العنوان الرئيسي (عربي)" value={String(settings.heroTitleAr)} onChange={(value) => setSettings({ ...settings, heroTitleAr: value })} /><Field label="Hero title (English)" value={String(settings.heroTitleEn)} onChange={(value) => setSettings({ ...settings, heroTitleEn: value })} /><Field area label="الوصف (عربي)" value={String(settings.heroDescriptionAr)} onChange={(value) => setSettings({ ...settings, heroDescriptionAr: value })} /><Field area label="Description (English)" value={String(settings.heroDescriptionEn)} onChange={(value) => setSettings({ ...settings, heroDescriptionEn: value })} /><Field label="نص الزر الرئيسي" value={String(settings.primaryCtaAr)} onChange={(value) => setSettings({ ...settings, primaryCtaAr: value })} /><Field label="رابط الزر الرئيسي" value={String(settings.primaryCtaHref)} onChange={(value) => setSettings({ ...settings, primaryCtaHref: value })} /><Field label="نص زر تسجيل الحرفي" value={String(settings.secondaryCtaAr)} onChange={(value) => setSettings({ ...settings, secondaryCtaAr: value })} /><Field label="رابط تسجيل الحرفي" value={String(settings.secondaryCtaHref)} onChange={(value) => setSettings({ ...settings, secondaryCtaHref: value })} /><Field area label="الإعلان النصي أسفل الصفحة" value={String(settings.announcementAr)} onChange={(value) => setSettings({ ...settings, announcementAr: value })} /></div></AdminPanel><AdminPanel title="الأقسام والسياسات"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{[["showCategories", "إظهار دليل المهن"], ["showFeaturedProviders", "إظهار المحترفين البارزين"], ["showLatestRequests", "إظهار أحدث الطلبات"], ["showHowItWorks", "إظهار كيف يعمل السوق"], ["showTrustBar", "إظهار شريط الثقة"], ["allowPublicRequests", "السماح بطلب خدمة"], ["allowProviderRegistration", "فتح تسجيل الحرفيين"]].map(([key, label]) => <Toggle key={key} label={label} checked={Boolean(settings[key])} onChange={(checked) => setSettings({ ...settings, [key]: checked })} />)}</div><div className="mt-5 grid gap-4 sm:grid-cols-3"><NumberField label="عدد المهن البارزة" value={Number(settings.featuredCategoryLimit)} onChange={(value) => setSettings({ ...settings, featuredCategoryLimit: value })} /><NumberField label="عدد المحترفين" value={Number(settings.featuredProviderLimit)} onChange={(value) => setSettings({ ...settings, featuredProviderLimit: value })} /><NumberField label="عدد الطلبات" value={Number(settings.latestRequestLimit)} onChange={(value) => setSettings({ ...settings, latestRequestLimit: value })} /></div></AdminPanel><AdminPanel title="قاعدة إرسال الطلبات" description="يصل الطلب إلى عدد محدود من المزودين لا إلى الجميع، لأن طلباً يصل إلى الجميع لا يستحق ردّ أحد. هذه الأرقام تسري على هذه الدولة وحدها."><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><NumberField label="عدد المزودين في كل موجة" value={Number(settings.matchWaveSize)} onChange={(value) => setSettings({ ...settings, matchWaveSize: value })} /><NumberField label="عدد مرات طلب مزودين آخرين" value={Number(settings.maxRequestRenewals)} onChange={(value) => setSettings({ ...settings, maxRequestRenewals: value })} /><NumberField label="مدة منع طالب الخدمة (أيام)" value={Number(settings.requestBlockDays)} onChange={(value) => setSettings({ ...settings, requestBlockDays: value })} /><NumberField label="صلاحية العرض (ساعات)" value={Number(settings.offerValidityHours)} onChange={(value) => setSettings({ ...settings, offerValidityHours: value })} /></div><p className="mt-4 text-sm text-gray-500 dark:text-gray-400">{`يصل الطلب إلى ${Number(settings.matchWaveSize)} مزودين، ويستطيع طالب الخدمة طلب غيرهم ${Number(settings.maxRequestRenewals)} مرة، أي ${Number(settings.matchWaveSize) * (Number(settings.maxRequestRenewals) + 1)} مزوداً على الأكثر. بعدها يُمنع من فتح طلب جديد ${Number(settings.requestBlockDays)} يوماً.`}</p></AdminPanel><button onClick={() => void saveSettings()} disabled={busy} className="inline-flex items-center gap-2 rounded-xl bg-[var(--color-primary)] px-5 py-3 text-sm font-bold text-white hover:bg-[var(--color-primary-hover)] disabled:opacity-50"><Save className="h-4 w-4" />حفظ ونشر التعديلات</button></section>}

        {!loading && tab === "categories" && <section className="grid items-start gap-5 xl:grid-cols-[1fr_380px]"><AdminPanel title="شجرة المهن والتصنيفات" description={`${categories.length} تصنيفًا — يمكن إخفاء أي مهنة أو إبرازها فورًا.`}><div className="space-y-4">{groups.map((group) => <div key={group.id} className="rounded-2xl border border-[var(--color-border)] dark:border-[var(--color-border)]"><div className="flex items-center justify-between bg-[var(--color-surface-muted)] px-4 py-3 dark:bg-[var(--color-surface)]/70"><div className="flex items-center gap-2"><ServiceCategoryIcon name={group.icon} className="h-5 w-5 text-[var(--color-primary)]" /><p className="font-bold text-[var(--color-text-primary)] dark:text-[var(--color-text-primary)]">{nameFor(locale, group.name_ar, group.name_en, group.name_tr, group.code)}</p></div><button onClick={() => editCategory(group)} className="text-xs font-bold text-[var(--color-primary)]">تعديل القسم</button></div><div className="divide-y divide-[var(--color-border)] dark:divide-[var(--color-border)]">{categories.filter((category) => category.parent_id === group.id).map((category) => <div key={category.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3"><div className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[var(--color-primary-soft)] text-[var(--color-primary)] dark:bg-[var(--color-primary-soft)]/40 dark:text-[var(--color-primary)]"><ServiceCategoryIcon name={category.icon} className="h-4 w-4" /></span><div><p className="text-sm font-bold text-[var(--color-text-primary)] dark:text-[var(--color-surface-muted)]">{nameFor(locale, category.name_ar, category.name_en, category.name_tr, category.code)}</p><p className="text-label text-[var(--color-text-muted)] font-medium">{category.booking_mode === "both" ? "حجز مباشر + عروض" : category.booking_mode === "instant" ? "حجز مباشر" : "طلب عروض"} · {Number(category.provider_count ?? 0)} محترف</p></div></div><div className="flex items-center gap-2"><button title="إبراز" aria-label={category.is_featured ? `إلغاء إبراز ${category.name_ar}` : `إبراز ${category.name_ar}`} aria-pressed={Boolean(category.is_featured)} onClick={() => void updateCategory(category, { isFeatured: !category.is_featured })} className={`rounded-lg p-2 ${category.is_featured ? "bg-[var(--color-warning-soft)] text-[var(--color-warning)]" : "bg-[var(--color-background)] text-[var(--color-text-muted)] dark:bg-[var(--color-surface)]"}`}><Star className="h-4 w-4" /></button><button onClick={() => void updateCategory(category, { isActive: category.is_active === 0 })} className={`rounded-lg px-2.5 py-1.5 text-label font-bold ${category.is_active === 0 ? "bg-[var(--color-error-soft)] text-red-600" : "bg-[var(--color-success-soft)] text-[var(--color-success)]"}`}>{category.is_active === 0 ? "مخفي" : "ظاهر"}</button><button onClick={() => editCategory(category)} className="rounded-lg bg-[var(--color-primary-soft)] px-2.5 py-1.5 text-label font-bold text-[var(--color-primary)]">تعديل</button><button onClick={() => removeCategory(category)} className="rounded-lg bg-[var(--color-error-soft)] px-2.5 py-1.5 text-label font-bold text-red-600">حذف</button></div></div>)}</div></div>)}</div></AdminPanel><AdminPanel id="category-editor" title={editingCategoryId ? "تعديل التصنيف" : "إضافة مهنة أو قسم"}><div className="space-y-3"><Field label="الرمز البرمجي" value={categoryForm.code} disabled={Boolean(editingCategoryId)} onChange={(value) => setCategoryForm({ ...categoryForm, code: value })} /><Field label="الاسم العربي" value={categoryForm.nameAr} onChange={(value) => setCategoryForm({ ...categoryForm, nameAr: value })} /><Field label="الاسم الإنجليزي" value={categoryForm.nameEn} onChange={(value) => setCategoryForm({ ...categoryForm, nameEn: value })} /><Field area label="وصف مختصر" value={categoryForm.descriptionAr} onChange={(value) => setCategoryForm({ ...categoryForm, descriptionAr: value })} /><label className="block text-xs font-bold text-[var(--color-text-secondary)] dark:text-[var(--color-text-muted)]">القسم الأب<select value={categoryForm.parentId} onChange={(event) => setCategoryForm({ ...categoryForm, parentId: event.target.value })} className={`${inputClass} mt-1`}><option value="">قسم رئيسي</option>{groups.filter((group) => group.id !== editingCategoryId).map((group) => <option key={group.id} value={group.id}>{group.name_ar || group.code}</option>)}</select></label><div className="grid grid-cols-2 gap-3"><Field label="اسم الأيقونة" value={categoryForm.icon} onChange={(value) => setCategoryForm({ ...categoryForm, icon: value })} /><Field label="شارة قصيرة" value={categoryForm.badgeAr} onChange={(value) => setCategoryForm({ ...categoryForm, badgeAr: value })} /></div><label className="block text-xs font-bold text-[var(--color-text-secondary)] dark:text-[var(--color-text-muted)]">طريقة الطلب<select value={categoryForm.bookingMode} onChange={(event) => setCategoryForm({ ...categoryForm, bookingMode: event.target.value as CategoryForm["bookingMode"] })} className={`${inputClass} mt-1`}><option value="quotes">طلب عروض</option><option value="instant">حجز مباشر</option><option value="both">الطريقتان</option></select></label><div className="grid grid-cols-2 gap-3"><Field label="السعر من" value={categoryForm.priceMin} onChange={(value) => setCategoryForm({ ...categoryForm, priceMin: value })} /><Field label="السعر إلى" value={categoryForm.priceMax} onChange={(value) => setCategoryForm({ ...categoryForm, priceMax: value })} /></div><Field label="ترتيب العرض" value={categoryForm.sortOrder} onChange={(value) => setCategoryForm({ ...categoryForm, sortOrder: value })} /><div className="grid grid-cols-2 gap-2"><Toggle label="يتطلب ترخيصًا" checked={categoryForm.requiresLicense} onChange={(value) => setCategoryForm({ ...categoryForm, requiresLicense: value })} /><Toggle label="يتطلب معاينة" checked={categoryForm.requiresVisit} onChange={(value) => setCategoryForm({ ...categoryForm, requiresVisit: value })} /><Toggle label="مميّز" checked={categoryForm.isFeatured} onChange={(value) => setCategoryForm({ ...categoryForm, isFeatured: value })} /><Toggle label="نشط" checked={categoryForm.isActive} onChange={(value) => setCategoryForm({ ...categoryForm, isActive: value })} /></div><div className="flex gap-2"><button onClick={() => void saveCategory()} disabled={busy || !categoryForm.code.trim()} className="flex-1 rounded-xl bg-[var(--color-primary)] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{editingCategoryId ? "حفظ التعديل" : "إضافة التصنيف"}</button>{editingCategoryId && <button onClick={() => { setEditingCategoryId(null); setCategoryForm(EMPTY_CATEGORY); }} className="rounded-xl border border-[var(--color-border)] px-3 text-sm font-bold dark:border-[var(--color-border)]">إلغاء</button>}</div></div></AdminPanel></section>}

        {!loading && tab === "providers" && <AdminPanel title="إدارة الحرفيين ومقدمي الخدمات" description="اعتماد الملفات، تعليقها، إبرازها في الواجهة أو إيقاف استقبال الطلبات."><div className="mb-4 flex flex-wrap gap-2">{[["", "الكل"], ["submitted", "مُرسل"], ["under_review", "قيد المراجعة"], ["approved", "معتمد"], ["rejected", "مرفوض"], ["suspended", "موقوف"]].map(([value, label]) => <button key={value} onClick={() => setProviderStatusFilter(value)} className={`rounded-xl px-3 py-2 text-xs font-bold ${providerStatus === value ? "bg-[var(--color-primary)] text-white" : "border border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-text-secondary)] dark:border-[var(--color-border)] dark:bg-[var(--color-surface)] dark:text-[var(--color-text-muted)]"}`}>{label}</button>)}</div><div className="overflow-x-auto"><table className="w-full min-w-[820px] text-sm"><thead><tr className="border-b border-[var(--color-border)] text-xs text-[var(--color-text-muted)] dark:border-[var(--color-border)]"><th className="px-3 py-3 text-start">مقدم الخدمة</th><th className="px-3 py-3 text-start">الموقع</th><th className="px-3 py-3 text-start">الحالة</th><th className="px-3 py-3 text-start">الظهور</th><th className="px-3 py-3 text-start">العمليات</th></tr></thead><tbody>{filteredProviders.map((provider) => <tr key={provider.id} className="border-b border-[var(--color-border)] last:border-0 dark:border-[var(--color-border)]"><td className="px-3 py-3"><Link href={`/providers/${provider.id}`} target="_blank" className="font-bold text-[var(--color-primary)] hover:underline dark:text-[var(--color-primary)]">{provider.business_name || nameFor(locale, provider.display_name_ar, provider.display_name_en, null, "مقدم خدمة")}</Link><p className="mt-0.5 text-label text-[var(--color-text-muted)] font-medium">{formatDate(provider.created_at)}</p></td><td className="px-3 py-3 text-[var(--color-text-muted)]">{provider.governorate || provider.city_id || "—"}</td><td className="px-3 py-3"><Status value={provider.status} /></td><td className="px-3 py-3"><div className="flex gap-2"><button aria-label={provider.is_featured ? `إلغاء إبراز ${provider.display_name_ar ?? provider.business_name}` : `إبراز ${provider.display_name_ar ?? provider.business_name}`} aria-pressed={Boolean(provider.is_featured)} onClick={() => void updateProvider(provider, { isFeatured: !provider.is_featured })} className={`rounded-lg p-2 ${provider.is_featured ? "bg-[var(--color-warning-soft)] text-[var(--color-warning)]" : "bg-[var(--color-background)] text-[var(--color-text-muted)] dark:bg-[var(--color-surface)]"}`}><Star className="h-4 w-4" /></button><button onClick={() => void updateProvider(provider, { isAcceptingRequests: !provider.is_accepting_requests })} className={`rounded-lg px-2 py-1 text-label font-bold ${provider.is_accepting_requests === 0 ? "bg-[var(--color-error-soft)] text-red-600" : "bg-[var(--color-success-soft)] text-[var(--color-success)]"}`}>{provider.is_accepting_requests === 0 ? "متوقف" : "يستقبل"}</button></div></td><td className="px-3 py-3"><div className="flex flex-wrap gap-1.5">{/* The moves PROVIDER_FLOW allows from where this provider stands, and nothing else. The buttons were derived from `!== approved` and `!== rejected`, so the screen offered moves the server refuses — a suspended provider back to approved, a draft straight to approved — and the refusal arrived as an unhandled 500. */}{PROVIDER_DECISIONS.filter((move) => canTransitionProvider(provider.status, move.to)).map((move) => <button key={move.to} onClick={() => decideProvider(provider, move)} className={`rounded-lg px-2.5 py-1.5 text-label font-bold ${move.tone === "danger" ? "bg-[var(--color-error-soft)] text-[var(--color-error)]" : "bg-[var(--color-success-soft)] text-[var(--color-success)]"}`}>{move.label}</button>)}{PROVIDER_DECISIONS.every((move) => !canTransitionProvider(provider.status, move.to)) && <span className="text-label font-bold text-[var(--color-text-muted)]">لا إجراء متاح</span>}</div></td></tr>)}</tbody></table>{filteredProviders.length === 0 && <p className="py-12 text-center text-sm font-bold text-[var(--color-text-muted)]">لا توجد ملفات في هذا القسم.</p>}</div></AdminPanel>}

        {!loading && tab === "operations" && <section className="grid gap-5 xl:grid-cols-2"><AdminPanel title="طلبات الخدمات" description={`${overview?.totalRequests ?? 0} طلب إجمالي — اقبل الطلب أو ارفضه أو اطلب معلومات أو أسنده لموظف`}><SimpleRows rows={snapshot.recentRequests} empty="لا توجد طلبات" render={(row) => <><div className="min-w-0"><Link href={`/service-requests/${String(row.id)}`} target="_blank" className="text-sm font-bold text-[var(--color-primary)] hover:underline dark:text-[var(--color-primary)]">{String(row.title || row.reference_number || "طلب خدمة")}</Link><p className="text-xs text-[var(--color-text-muted)]">{String(row.category_name_ar || "غير مصنف")} · {formatDate(String(row.created_at || ""))}</p>{/* The reviewer's own note is what the customer was told; it belongs beside the decision, not only in the audit log. */}{Boolean(row.review_note) && <p className="mt-1 text-label text-[var(--color-text-secondary)] font-medium">«{String(row.review_note)}»</p>}{Boolean(row.assigned_to) && <p className="mt-1 text-label font-bold text-[var(--color-text-muted)]">المتابع: {String(row.assigned_to)}</p>}<div className="mt-2 flex flex-wrap gap-1.5">{REQUEST_DECISIONS.map((move) => <button key={move.action} onClick={() => decideRequest(row, move)} className={`rounded-lg px-2.5 py-1.5 text-label font-bold ${move.tone === "danger" ? "bg-[var(--color-error-soft)] text-[var(--color-error)]" : "bg-[var(--color-primary-soft)] text-[var(--color-primary)]"}`}>{move.label}</button>)}</div></div><div className="shrink-0 text-end"><Status value={String(row.review_status || REQUEST_REVIEW_STATUS.PENDING)} /><p className="mt-1 text-label text-[var(--color-text-muted)] font-medium">دورة الطلب: {STATUS_LABELS[String(row.status || "")] ?? String(row.status || "—")}</p><p className="mt-1 text-label text-[var(--color-text-muted)] font-medium">{row.budget_min == null ? "ميزانية مفتوحة" : `${String(row.budget_min)}–${String(row.budget_max || "")} ${currencyLabel(row.currency)}`}</p></div></>} /></AdminPanel><AdminPanel title="المهام والأوامر" description={`${overview?.totalJobs ?? 0} مهمة إجمالية`}><SimpleRows rows={snapshot.recentOrders} empty="لا توجد مهام" render={(row) => <><div><p className="text-sm font-bold text-[var(--color-text-primary)] dark:text-[var(--color-surface-muted)]">{String(row.request_title || row.reference_number || "مهمة خدمة")}</p><p className="text-xs text-[var(--color-text-muted)]">{formatDate(String(row.created_at || ""))} · {row.agreed_price == null ? "—" : `${String(row.agreed_price)} ${currencyLabel(row.currency)}`}</p></div><Status value={String(row.status || "")} /></>} /></AdminPanel></section>}

        {!loading && tab === "reports" && <AdminPanel title="البلاغات والمراجعة" description="معالجة بلاغات الطلبات والملفات والمحادثات."><SimpleRows rows={reports} empty="لا توجد بلاغات" render={(report) => <><div><p className="text-sm font-bold text-[var(--color-text-primary)] dark:text-[var(--color-surface-muted)]">{report.reason || report.description || "بلاغ"}</p><p className="text-xs text-[var(--color-text-muted)]">{report.target_type} · {formatDate(report.created_at)}</p></div><div className="flex items-center gap-2"><Status value={report.status || "open"} />{(report.status === "open" || report.status === "in_review") && <button onClick={() => resolveReport(report)} disabled={busy} className="rounded-lg bg-[var(--color-primary)] px-3 py-1.5 text-label font-bold text-white">معالجة</button>}</div></>} /></AdminPanel>}
      </main>
      <DecisionDialog
        decision={decision}
        busy={busy}
...
function DecisionDialog({
  ...
  // reset during render when the decision changes, not in an effect.
  ...
  return (
    <Dialog open onClose={onClose} title={decision.title} closeLabel="إغلاق" size="md">
      ...
      <div className="mt-5 flex justify-end gap-2">
        <button type="button" onClick={onClose} className="rounded-xl border border-[var(--color-border)] px-4 py-2.5 text-sm font-bold text-[var(--color-text-secondary)]">
          إلغاء
        </button>
        <button
          type="button"
          ...
          className={`rounded-xl px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50 ${decision.tone === "danger" ? "bg-[var(--color-error)]" : "bg-[var(--color-primary)]"}`}
        >
