...
function OfficeBranchesPageContent() {
  ...
  useEffect(() => {
    fetch(`/api/office/branches${organizationId ? `?org=${encodeURIComponent(organizationId)}` : ''}`)
      .then(res => res.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/office/branches${organizationId ? `?org=${encodeURIComponent(organizationId)}` : ''}`, {
  ...
  const deleteBranch = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/office/branches?id=${id}${organizationId ? `&org=${encodeURIComponent(organizationId)}` : ''}`, { method: 'DELETE' });
