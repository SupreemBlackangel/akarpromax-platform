            ...
            {deliveries.map((d) => (
              <li key={d.id} className="rounded-2xl border border-gray-200 bg-[var(--color-surface)] p-4 dark:border-gray-800 dark:bg-gray-900">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-label font-bold uppercase ${STATUS_TONES[d.status] ?? "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300"}`}>
                    {d.status}
                  </span>
                </div>
              ...
              {rules.map((rule) => (
                <tr key={String(rule.id)}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-label font-bold uppercase ${Number(rule.enabled) === 1 ? "bg-emerald-100 text-[var(--color-success)] dark:bg-[var(--color-success-soft)]/40 dark:text-[var(--color-success)]" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}>
