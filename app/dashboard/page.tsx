      ...
      <div dir={dir} className="container py-8">
        {!viewer.authenticated ? (
          <div className="max-w-md mx-auto text-center py-24">
            ...
            <h1 className="text-2xl font-black text-gray-900 dark:text-[var(--color-surface)] mb-2">
            ...
            <button
              ...
              className="px-6 py-3 rounded-xl bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white text-sm font-bold transition"
            >
          ...
          <>
            <div className="mb-6">
              <h1 className="text-2xl font-black text-gray-900 dark:text-[var(--color-surface)]">{t("services.dashboard") ?? "لوحة التحكم"}</h1>
            ...
            <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {cards.map((card) => (
                <Link
                  key={card.href}
                  href={card.href}
                  className="group bg-[var(--color-surface)] dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 transition hover:border-[var(--color-primary)]/30 dark:hover:border-blue-700 hover:shadow-md"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-3xl">{card.icon}</span>
                    <span className="text-[var(--color-primary)] transition-transform group-hover:translate-x-1" dir="ltr">→</span>
                  </div>
                  <p className="mt-3 font-black text-gray-900 dark:text-[var(--color-surface)] group-hover:text-[var(--color-primary)] dark:group-hover:text-blue-400">
                    {card.title}
                  </p>
              ...
              <Link
                ...
                className="group flex flex-col justify-center items-center bg-[var(--color-primary-soft)] dark:bg-[var(--color-primary-soft)]/20 border border-blue-100 dark:border-blue-800 rounded-2xl p-5 transition hover:border-[var(--color-primary)]/30 hover:shadow-md"
              >
                <span className="text-3xl">➕</span>
                <span className="mt-2 font-black text-[var(--color-primary)] dark:text-[var(--color-primary)]">
