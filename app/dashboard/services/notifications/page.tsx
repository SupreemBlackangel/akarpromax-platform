    ...
    <ServiceDashboardShell viewer={viewer} locale={locale} dir={dir} t={t} active="notifications">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-[var(--color-text-primary)]">{t("services.notifications") ?? "التنبيهات"}</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-[var(--color-text-primary)]">{t("services.noNotifications") ?? "لا توجد تنبيهات"}</h2>
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="shrink-0 text-label text-gray-400 font-medium">{new Date(n.created_at).toLocaleString(locale === "ar" ? "ar-SA" : locale === "tr" ? "tr-TR" : "en-US")}</span>
                  </div>
                  {n.body && <p className="mt-1 text-sm text-gray-500 dark:text-gray-400 line-clamp-2">{n.body}</p>}
                  {!n.read && <span className="mt-2 inline-block px-2 py-0.5 text-label font-bold text-white bg-[var(--color-primary)] rounded-full">{t("services.new") ?? "جديد"}</span>}
                </div>
