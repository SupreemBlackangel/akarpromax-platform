...
function CompanyProfilePageContent() {
  ...
  useEffect(() => {
    fetch(`/api/company/profile${organizationId ? `?org=${encodeURIComponent(organizationId)}` : ''}`)
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          const d = data.data;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/company/profile${organizationId ? `?org=${encodeURIComponent(organizationId)}` : ''}`, {
        method: 'PATCH',
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {profile.specialties.map((s, i) => (
                <span key={i} className="inline-flex items-center gap-1 px-3 py-1 bg-[var(--color-primary-soft)] text-[var(--color-primary)] rounded-full text-sm">
                  {s}
