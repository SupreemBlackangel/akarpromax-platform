...
function serialiseAdvertiser(row: AdvertiserRow) {
...
async function writeAudit(db: D1Database, actor: string | null, action: string, advertiserId: string, metadata: object) {
  await db.prepare(
    `INSERT INTO audit_logs (id, actor_user_id, action, entity_type, entity_id, metadata)
     VALUES (?1, ?2, ?3, 'advertiser', ?4, ?5)`,
  )
    .bind(crypto.randomUUID(), actor, action, advertiserId, JSON.stringify(metadata))
    .run();
}
...
export async function GET(request: NextRequest) {
  ...
  const advertisers = rows.results.map(serialiseAdvertiser);
...
export async function POST(request: NextRequest) {
  ...
  if (!countryCode || !nameAr || !nameEn || !nameTr || !canManageCountry(identity, countryCode)) {
    return NextResponse.json({ error: "Invalid advertiser data or country scope" }, { status: 400 });
  }

  const requestedStatus = normaliseChoice(body.status, allowedStatuses, "draft");
  const status =
    requestedStatus === "active" && !hasPermission(identity, PERMISSIONS.ADVERTISERS_APPROVE)
      ? "draft"
      : requestedStatus;
  ...
  await writeAudit(db, identity.email, "advertiser.created", id, { countryCode, status });
...
export async function PATCH(request: NextRequest) {
  ...
  if (!id || !nameAr || !nameEn || !nameTr) {
    return NextResponse.json({ error: "Advertiser names are required" }, { status: 400 });
  }
  const db = await getRuntimeDb();
  ...
  const requestedStatus = normaliseChoice(body.status, allowedStatuses, "draft");
  if (requestedStatus === "active" && !hasPermission(identity, PERMISSIONS.ADVERTISERS_APPROVE)) {
  ...
  await writeAudit(db, identity.email, "advertiser.updated", id, { status: requestedStatus });
...
export async function DELETE(request: NextRequest) {
  ...
  await db.prepare(
    ...
    .bind(id)
    .run();
  await writeAudit(db, identity.email, "advertiser.archived", id, {});
  return NextResponse.json({ ok: true });
}
