...
import type { SponsorUser } from "@/src/types/sponsor";
...
function serialise(row: Record<string, unknown>): SponsorUser {
  return {
    id: String(row.id),
...
export async function POST(request: NextRequest) {
  ...
  if (!sponsorId || !email) {
    return NextResponse.json({ error: "sponsorId and email are required" }, { status: 400 });
  }
  ...
  ).bind(sponsorId, email).first();
  if (existing) {
    return NextResponse.json({ error: "User with this email already exists for this sponsor" }, { status: 409 });
  }
