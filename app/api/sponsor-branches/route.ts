...
import type { SponsorBranch } from "@/src/types/sponsor";
...
function serialise(row: Record<string, unknown>): SponsorBranch {
  return {
    id: String(row.id),
...
export async function POST(request: NextRequest) {
  ...
  if (!sponsorId || !nameAr || !nameEn || !countryCode || !cityId) {
    return NextResponse.json({ error: "sponsorId, nameAr, nameEn, countryCode, cityId are required" }, { status: 400 });
  }
  ...
  await db.prepare(
    ...
    id, sponsorId, nameAr, nameEn, countryCode, cityId,
