...
function serialise(row: Record<string, unknown>): SponsorProfile {
  return {
    id: String(row.id),
    sponsorCode: String(row.sponsor_code),
    ...
    status: String(row.status) as SponsorProfile["status"],
...
export async function POST(request: NextRequest) {
  ...
  const sponsorCode = `SP-${id.slice(0, 8).toUpperCase()}`;
  const requestedStatus = normaliseChoice(body.status, allowedStatuses, "draft");
  const status = requestedStatus === "active" && !hasSponsorPermission(identity, PERMISSIONS.SPONSORS_APPROVE) ? "draft" : requestedStatus;
...
export async function PATCH(request: NextRequest) {
  ...
  const status = requestedStatus === "active" && !hasSponsorPermission(identity, PERMISSIONS.SPONSORS_APPROVE)
