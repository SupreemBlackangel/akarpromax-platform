...
import { withdrawOffer } from "@/lib/services/marketplace";
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    await withdrawOffer(id, identity.email, { userId: identity.email, ip: request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ?? null });
  } catch (error) {
